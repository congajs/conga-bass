...
class CongaBass {
    ...
    onKernelCompile(event, next) {
        ...
        // attach logger
        if (config.logging instanceof Object) {
            // dynamically replace the logger SID (@logger.default) with its instance
            Object.keys(config.logging).forEach(key => {
                const sid = config.logging[key].substring(1);
                if (container.has(sid)) {
                    config.logging[key] = container.get(sid);
                }
            });
        }
    ...
    findBundleNameFromPath(filePath) {
        ...
        if (namespace.length === 0) {
            let matches = filePath.match(/(@conga\/framework-[^\/]+)/);
            if (matches.length >= 2) {
                namespace = matches[1];
            }
        }
