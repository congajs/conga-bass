...
class BassListener {
        ...
        config.adapters.forEach(function(adapter, index){
            ...
            if (fs.existsSync(p)) {
                ...
                try {
                    ...
                    throw new Error('bass adapter: ' + adapter + " can't be found!");
                }
    ...
    onRequestScope(event, next) {
        ...
        if (scopeEnabled && (config.request_scope || (config.request_scope === undefined &&
             ...
             */
            container.set('bass', new BassRequestProxy(container.get('bass'), event.request, container));
        }
        next();
    }
